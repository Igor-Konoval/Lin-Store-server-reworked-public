...
class OldViewsController {
	...
	async addProduct(req, res) {
		try {
			...
			const oldViewsResult = await OldViewsModel.findOne({
				userId: token.userId,
			})
	...
	async updateOldViews(req, res) {
		try {
			...
			const updateOldViews = await OldViewsModel.findOne({
				userId: token.userId,
			})
			if (!updateOldViews) {
			...
			updateOldViews.productsId = updateOldViews.productsId.filter(
				id => !id.equals(new ObjectId(isValidProductId))
			)
