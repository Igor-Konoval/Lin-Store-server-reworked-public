...
class ProductController {
	async getOneProduct(req, res) {
		try {
			...
			const product = await ProductModel.findOne({ name: isValidProductName })
				.populate('typeId', 'name')
				.populate('brandId', 'name')
			const newProduct = {
				...product.toObject(),
				typeId: product.typeId.name,
				brandId: product.brandId.name,
			}
	...
	async createProduct(req, res) {
		try {
			...
			async function saveImageToGCS(file, folder) {
				const bucketName = 'lin-store-bucket'
						...
						if (Array.isArray(req.body.colors)) {
							for (const colorObj of req.body.colors) {
								...
								if (req.files[imageKey]) {
									...
									const publicNameFile = await saveImageToGCS(
										imgColor,
										'colors'
									)
									colors.push({
										color: colorObjValue.color,
										count: colorObjValue.count,
										urlImg: publicNameFile,
									})
								}
							}
						} else {
	...
	async getAllProduct(req, res) {
		const searchTerm = sanitizedData(req.query.search)
		const page = sanitizedData(parseInt(req.query.p)) || 1
		...
		try {
			...
			let findMinPrice = (fixedMin = await ProductModel.find()
				.sort({ price: 1 })
				.limit(1)
				.select('price'))
			let findMaxPrice = (fixedMax = await ProductModel.find()
				.sort({ price: -1 })
				.limit(1)
				.select('price'))
			...
			if (!searchTerm) {
				...
				if (isType) {
					...
					productList = fuse.search(searchTerm).map(value => value.item)
				}

				if (typeId && brandId) {
					productList = productList.filter(
						product =>
							product.typeId === typeId.toString() &&
							product.brandId === brandId.toString()
					)
				} else if (!typeId && brandId) {
					productList = productList.filter(
					...
					productList = productList.filter(
						product => product.typeId === typeId.toString()
					)
				}
			}
