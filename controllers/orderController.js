...
class OrderController {
	async getOrder(req, res) {
		try {
			...
			const fetchWithRetry = async product => {
				...
				throw new Error(
					`Failed to fetch TTN ${product.TTN} after ${maxRetries} retries`
				)
			}
	...
	async cancelOrder(req, res) {
		try {
			...
			const order = await OrderModel.find({
				TTN: isValidTTN,
				orderNumber: isValidOrderNumber,
			})
			...
			for (const product of orderProducts) {
				...
				if (productToUpdate) {
					const colorToUpdate = productToUpdate.colors.find(
						c => c.color === color
					)
	...
	async getStreet(req, res) {
		try {
			...
			if (isValidStreet.length === 0 || isValidCityRef.length === 0) {
				return res.json({ message: 'Виникла помилка' })
			}
	...
	async createDocument(req, res) {
		try {
			...
			let i = 0
			for (let x = 0; x < isValidProductList.length; x++) {
				const findColor = products[i].colors.find(
					item => item.color === isValidProductList[x].selectedColor
				)

				const isValidCount =
					isValidProductList[x].selectedCount <= findColor.count

				if (!isValidCount) {
					return res.json(`${products[i].name} немає в наявності :(`)
				}

				if (isValidProductList[x + 1]?._id === products[i]._id) {
					x++
				} else {
					x++
					i++
			...
			const productsArrObjs = isValidProductList.map(value => ({
				_id: value._id,
				color: value.selectedColor,
				count: value.selectedCount,
			}))

			const totalPrice = isValidProductList.reduce(
				(prev, cur) => prev + cur.price,
				0
			)

			const updateProducts = isValidProductList.map(value => ({
				_id: value._id,
				name: value.name,
				img: value.img,
			...
			const counterparty = await axios.post(
				'https://api.novaposhta.ua/v2.0/json/',
				{
					apiKey: process.env.API_KEY,
			...
			const descriptionProducts = isValidProductList
				.map(value => value.name + ' ' + value.selectedColor)
				.join(', ')
			...
			const result = await axios.post('https://api.novaposhta.ua/v2.0/json/', {
				...
				calledMethod: 'save',
				methodProperties: {
					SenderWarehouseIndex: '91/13', // "101/102",
					RecipientWarehouseIndex: isValidRecipientsWarehouse, //"91/13",// "101/102",
					PayerType: 'Recipient',
					PaymentMethod: 'Cash',
					// DateTime : "дд.мм.рррр",
					CargoType: 'Parcel',
			...
			const createOrdersPromises = allAdmins
				.concat(allGoogleAdmins)
				.map(async admin => {
                    ...
                    <!DOCTYPE html>
                        <html lang="en">
                            ...
                            <body style="font-family: 'Arial', sans-serif; margin: 0; padding: 0; background-color: #f4f4f4;">
                                <div style="max-width: 600px; margin: 20px auto; padding: 20px; background-color: #ffffff; border-radius: 10px; box-shadow: 0 0 10px rgba(145,141,141,0.1);">
                                    <h2>Номер накладной ${
																			result.data.data[0].IntDocNumber
																		}</h2>
                                    <h2>Номер заказа на сайте ${orderNumber}</h2>
                                    ${updateProducts.map(
																			product =>
                                    ...
                                    <div style="text-align: right; margin-top: 18px">
                                        <a href="${
																				...
																				}order/acceptOrder/${orderNumber}" style="background-color: rgb(188 188 100); border-radius: 12px; color: black; padding: 10px; font-size: 25px; text-decoration: none; ">подтвердить</a>
                                        <a href="${
																					process.env.CLIENT_APP
																				}order/rejectOrder/${orderNumber}" style="background-color: rgb(188 188 100); border-radius: 12px; color: black; padding: 10px; font-size: 25px; margin-left: 30px; text-decoration: none">отменить</a>
                                    </div>
	...
	async acceptOrder(req, res) {
		try {
			...
			const isGoogleAdmin = await GoogleUserModel.findOne({
				_id: token.userId,
				role: token.role,
			})
	...
	async rejectOrder(req, res) {
		try {
			...
			const checkOrder = await OrderModel.findOne({ orderNumber })
			if (
				checkOrder.info[checkOrder.info - 1] ===
					'скасовано продавцем через нестачу на складі' ||
	...
	async checkDetails(req, res) {
		try {
			...
			const resultCost = await axios.post(
				'https://api.novaposhta.ua/v2.0/json/',
				{
					apiKey: process.env.API_KEY,
					modelName: 'InternetDocument',
			...
			const resultDelivery = await axios.post(
				'https://api.novaposhta.ua/v2.0/json/',
				{
					apiKey: process.env.API_KEY,
					modelName: 'InternetDocument',
	...
	async checkOrderForCom(req, res) {
		try {
			...
			if (!checkOrder) {
				return res.json({
					message: { isGetProduct: false, isSetRating: false },
				})
			}
			...
			const checkRating =
				isExistComment.filter(comment => comment.rating > 0).length > 0

			const checkStatus = () => {
				if (
					checkOrder.status === 9 ||
					checkOrder.status === 10 ||
					checkOrder.status === 11
				) {
					return true
				} else {
					return false
